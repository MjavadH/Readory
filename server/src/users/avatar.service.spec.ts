...
describe('AvatarService', () => {
  ...
  it('rejects PNG, corrupted, oversized, and too-large-dimension uploads', async () => {
    ...
    await expect(service.processAvatar(file(Buffer.from('not an image')))).rejects.toBeInstanceOf(
      BadRequestException,
    );
    await expect(service.processAvatar(file(Buffer.alloc(5 * 1024 * 1024)))).rejects.toBeInstanceOf(
      BadRequestException,
    );
    const large = await sharp({
      create: { width: 1025, height: 10, channels: 3, background: '#fff' },
    })
      .jpeg()
      .toBuffer();
  ...
  it('replaces the avatar safely and deletes only the exact previous key', async () => {
    ...
    prisma.user.findUnique.mockResolvedValue({
      ...
      email: 'a@b.c',
      username: 'reader',
      avatarKey: 'media/avatar/7/11111111-1111-4111-8111-111111111111.webp',
      updatedAt: new Date(),
    });
  ...
  it('cleans up the new object and preserves the old avatar on concurrent update', async () => {
    ...
    await expect(service.replaceAvatar(7, file(Buffer.from('x')))).rejects.toBeInstanceOf(
      ConflictException,
    );

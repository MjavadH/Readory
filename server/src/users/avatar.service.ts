        ...
        try {
            ...
            if (user.avatarKey) {
                await this.storage.deleteKeys([user.avatarKey]).catch((err) => {
                    this.logger.error(`Failed to delete previous avatar ${user.avatarKey}: ${(err as Error).message}`);
                });
            }
            ...
            if (uploaded) {
                await this.storage.deleteKeys([newKey]).catch((err) => {
                    this.logger.error(`Failed to clean up orphan avatar ${newKey}: ${(err as Error).message}`);
                });
            }
            if (error instanceof BadRequestException || error instanceof ConflictException) throw error;
            this.logger.error(`Avatar replacement failed for user ${userId}: ${(error as Error).message}`);

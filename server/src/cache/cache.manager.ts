...
export class CacheManager {
    ...
    async setString(key: string, value: string, ttlSeconds: number, jitterPercent = 0.1): Promise<void> {
        const safeKey = this.sanitizeSegment(key);
        const start = Date.now();
    ...
    async getOrSet<T>(key: string, options: CacheGetOrSetOptions, loader: () => Promise<T>): Promise<T> {
        const safeKey = this.sanitizeSegment(key);
        const start = Date.now();
    ...
    private async writeEnvelope<T>(safeKey: string, value: T, options: CacheGetOrSetOptions): Promise<void> {

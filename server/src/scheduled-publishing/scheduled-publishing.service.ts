...
export class ScheduledPublishingService implements OnModuleInit, OnModuleDestroy {
    ...
    private async enqueueSchedule(schedule: ScheduledPublication) {
        ...
        await this.queue.add('publish', {scheduleId: schedule.id}, {
    ...
    private async publishSchedule(id: number, immediate = false, actorId?: number) {
        ...
        try {
            ...
            this.audit(immediate ? 'SCHEDULE_PUBLISHED_IMMEDIATELY' : 'SCHEDULE_PUBLISHED', schedule, actorId, schedule, {
                ...schedule,
    ...
    private async assertTargetExists(type: ScheduledTargetType, id: number) {
        const found = type === ScheduledTargetType.BOOK ? await this.prisma.book.findUnique({where: {id}}) : type === ScheduledTargetType.Chapter ? await this.prisma.chapter.findUnique({where: {id}}) : null;
        if (!found) throw new NotFoundException('Scheduled content not found');
    }

    private async markContentStatus(tx: Prisma.TransactionClient, type: ScheduledTargetType, id: number, status: PublicationStatus) {
        if (type === ScheduledTargetType.BOOK) await tx.book.update({
            where: {id},
            data: {publishStatus: status}
        }); else if (type === ScheduledTargetType.Chapter) await tx.chapter.update({
            where: {id},
            data: {publishStatus: status}
        });
    }

    private async publishTarget(tx: Prisma.TransactionClient, type: ScheduledTargetType, id: number, now: Date) {
        if (type === ScheduledTargetType.BOOK) await tx.book.update({
            where: {id},
            ...
            await tx.book.update({where: {id: chapter.bookId}, data: {lastContentUpdate: now}});
        } else throw new BadRequestException('Unsupported scheduled target type');
    }

    private async invalidate() {
        await Promise.all([this.publicService.clearHomeCache(), this.cache.del('books:browse:default'), this.cache.del('stats:books'), this.cache.del('stats:chapters:count')]);
    }

    private audit(action: string, schedule: ScheduledPublication, actorId?: number, before?: any, after?: any) {
        this.auditLog.log({
    ...
    private async withTargetName(item: ScheduledPublication) {
        const target = item.targetType === ScheduledTargetType.BOOK ? await this.prisma.book.findUnique({
            where: {id: item.targetId},
            select: {title: true}
        }) : item.targetType === ScheduledTargetType.Chapter ? await this.prisma.chapter.findUnique({
            where: {id: item.targetId},
            select: {title: true}
        }) : null;

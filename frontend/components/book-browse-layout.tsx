    ...
    >
      ...
      {icon && (
        <span
          className={cn(
            'shrink-0',
            checked ? 'text-primary' : 'text-muted-foreground group-hover:text-foreground/60',
          )}
        >
          {icon}
        </span>
            ...
            <div className="flex flex-wrap gap-2">
              {availableTypes.map((type) => (
                <FilterChip
                  ...
                  id={`type-${type.slug}`}
                  label={type.name}
                  icon={<AppIcon name={type.iconKey} className="h-3.5 w-3.5" />}
            ...
            <div className="flex flex-wrap gap-2">
              {availableGenres.map((genre) => (
                <FilterChip
                  ...
                  id={`genre-${genre.slug}`}
                  label={genre.name}
                  icon={<AppIcon name={genre.iconKey} className="h-3.5 w-3.5" />}
...
export function BookBrowseLayout({
  ...
  const activeCount = filters.selectedTypes.length + filters.selectedGenres.length;
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <div className="flex items-center gap-2">
              <Select
                ...
                onValueChange={(v) => filters.setSortBy(v as SortOption)}
              >
                ...
                <DrawerContent className="max-h-[92dvh]">
                        ...
                        {filters.selectedGenres.map((s) => {
                          ...
                          return g ? (
                            ...
                            >
                              ...
                              <button
                                type="button"
                                onClick={() => filters.handleGenreToggle(s)}
                                className="ms-0.5 inline-flex h-4 w-4 items-center justify-center rounded-full text-muted-foreground transition hover:bg-muted-foreground/20 hover:text-foreground"
                              >
                                <X className="h-2.5 w-2.5" />
                              </button>
                  ...
                  <DrawerFooter className="gap-2 border-t border-border/60 bg-background/95 px-5 py-4 backdrop-blur">
                    <div className="flex gap-2">
                      ...
                      <DrawerClose asChild>
                        <Button
                          className={cn('rounded-xl', hasActiveFilters ? 'flex-1' : 'w-full')}
                        >
                          {t('ShowResults')}
                        </Button>
                      </DrawerClose>
            ...
            <div className="sticky top-20 flex max-h-[calc(100vh-6rem)] flex-col overflow-hidden rounded-2xl border border-border/60 bg-card/70 shadow-sm backdrop-blur">
              ...
              <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain scrollbar-thin">
                <div className="space-y-1 p-3">
                  ...
                  {enableTypeFilter && enableGenreFilter && <Separator className="my-1" />}
          ...
          <main className="min-w-0 flex-1">
            {isLoading ? (
              ...
              <div className="flex min-h-105 flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-card/40 p-10 text-center">
                ...
                <h3 className="mt-4 text-lg font-semibold text-foreground">{t('NoBooksFound')}</h3>
                {hasActiveFilters && (
                  <Button
                    variant="outline"
                    onClick={filters.clearFilters}

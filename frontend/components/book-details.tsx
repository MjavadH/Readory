...
export type BookDetailsProps = {
  ...
  /** Hide the action button (admin page typically hides it). */
...
export function BookDetails({
  ...
  useEffect(() => {
    ...
    let alive = true;
    apiClient
      .get<{ subscribed: boolean }>(`/notifications/books/${book.id}/subscription`, {
        cache: 'no-store',
      })
      .then((res) => {
        if (alive) setChapterSubscribed(res.subscribed);
      })
      .catch(() => undefined);
    return () => {
      alive = false;
          ...
          <div className="grid gap-6 sm:gap-8 lg:grid-cols-[280px_1fr] lg:gap-12">
            ...
            >
              ...
              >
                ...
                {isAuthenticated && !hideActionButton && onAddToCollection && (
                  <motion.div whileTap={{ scale: 0.92 }} whileHover={{ scale: 1.04 }}>
                    <ActionTooltip content={t('AddToCollection')}>
                      <Button
                        variant="outline"
                        size="lg"
                ...
                {isAuthenticated && !hideActionButton && onToggleFavorite && (
                  <motion.div whileTap={{ scale: 0.92 }} whileHover={{ scale: 1.04 }}>
                    <ActionTooltip
                      content={t(isFavorited ? 'RemoveFromFavorites' : 'AddToFavorites')}
                    >
                      <Button
                        variant="outline"
                        size="lg"
                ...
                {isAuthenticated && !hideActionButton && (
                  <motion.div whileTap={{ scale: 0.92 }} whileHover={{ scale: 1.04 }}>
                    <ActionTooltip
                      content={t(chapterSubscribed ? 'Unsubscribe' : 'NewChapterNotification')}
                    >
                      <Button

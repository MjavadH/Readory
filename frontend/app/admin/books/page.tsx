...
export default function AdminBooks() {
  ...
  const fetchBooks = async () => {
    try {
      ...
      const transformedBooks: BookCardData[] = (data.books || []).map(
        (book) => ({
          id: book.id,
          title: book.title,
  ...
  const handleAddBook = async () => {
    ...
    try {
      await apiClient.post("/books", {
        ...newBook,
        alternativeTitles: newBook.alternativeTitlesText
          .split("\n")
          .map((v) => v.trim())
          .filter(Boolean),
        translators: newBook.translatorsText
          .split("\n")
          .map((v) => v.trim())
          .filter(Boolean),
        publicationYear: newBook.publicationYear
          ? Number(newBook.publicationYear)
          : undefined,
        ageRating: newBook.ageRating || undefined,
          ...
          <Card className="border-2 border-primary/20 shadow-lg">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="add-original-title">Original Title</Label>
                  <Input
                    id="add-original-title"
                    value={newBook.originalTitle}
                    onChange={(e) =>
                      setNewBook({ ...newBook, originalTitle: e.target.value })
                    }
                  />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="add-publication-year">Publication Year</Label>
                  <Input
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setNewBook({ ...newBook, status: v })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="NONE">{g("None")}</SelectItem>
                      {["GENERAL", "TEEN", "MATURE", "ADULT"].map((v) => (
                        <SelectItem key={v} value={v}>
                          {e(`AgeRating.${v}`)}
                        </SelectItem>
                      ))}
                ...
                <div className="space-y-2 md:col-span-2">
                  <Label htmlFor="add-alt-titles">
                    Alternative Titles (one per line)
                  </Label>
                  <Textarea
                    id="add-alt-titles"
                    value={newBook.alternativeTitlesText}
                    onChange={(e) =>
                      setNewBook({
                        ...newBook,
                        alternativeTitlesText: e.target.value,
                      })
                    }
                    rows={2}
                  />
                ...
                <div className="space-y-2 md:col-span-2">
                  <Label htmlFor="add-translators">
                    Translators (one per line)
                  </Label>
                  <Textarea
                    id="add-translators"
                    value={newBook.translatorsText}
                    onChange={(e) =>
                      setNewBook({
                        ...newBook,
                        translatorsText: e.target.value,
                      })
                    }
                    rows={2}
                  />
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={
                      newBook.typeId != null
                        ? String(newBook.typeId)
                        : undefined
                    }
                    onValueChange={(v) =>
                      setNewBook({ ...newBook, typeId: Number(v) })
                    }
              ...
              <div className="flex items-center space-x-2 p-3 bg-muted/30 rounded-lg">
                <input
                  ...
                  onChange={(e) =>
                    setNewBook({ ...newBook, isPublished: e.target.checked })
                  }
                  className="w-4 h-4"
                />
              ...
              <div className="flex items-center space-x-2 p-3 bg-muted/30 rounded-lg">
                <input
                  ...
                  onChange={(e) =>
                    setNewBook({ ...newBook, isFeatured: e.target.checked })
                  }
                  className="w-4 h-4"
                />

...
export function CollectionsClient({ initialData }: { initialData: CollectionsResponse }) {
    ...
    const loadMore = React.useCallback(async () => {
        ...
        try {
            const res = await apiClient.get<CollectionsResponse>(`/collections?limit=24&cursor=${encodeURIComponent(nextCursor)}`)
    ...
    return items.length > 0 ? (
        <>
            ...
            {hasMore && (
                <div ref={loadMoreRef} className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {isLoadingMore && Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-72 rounded-3xl" />)}
                </div>
            )}
        </>
    ) : (
